 ...
 * @since	0.1
 */
class JoomleagueModeljlextcountry extends JoomleagueModelItem
{
	/**
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query='SELECT * FROM #__joomleague_countries WHERE id='.(int) $this->_id;
			$this->_db->setQuery($query);
	...
	function _initData()
	{
      ...
      $league->checked_out_time	= 0;
			$league->itu				= null;
			$league->fips			= null;
			$league->picture			= 'images/com_joomleague/database/placeholders/placeholder_flags.png';
      $league->ioc		= null;
			$league->fifa			= null;
			$league->ds			= null;
			$league->wmo		= null;
	...
	function addLeague($newLeague)
	{
		...
		$query="SELECT name FROM #__joomleague_countries WHERE name='$newLeague'";
		$this->_db->setQuery($query);
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'jlextcountry', $prefix = 'table', $config = array())
	{
