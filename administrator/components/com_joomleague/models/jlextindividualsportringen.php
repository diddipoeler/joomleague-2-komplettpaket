 ...
 * @since	1.5.0a
 */
class JoomleagueModeljlextindividualsportringen extends JModel
{
	var $_datas=array();
	var $_league_id=0;
	var $_person_id=1;
	var $_matchplayer_id=1;	
    var $_lfdsinglematch = 1;
	var $_season_id=0;
	...
	var $_playerposition = array();
  var $_tempexportplayer = array();
  var $_tempexportteamplayer = array();
  var $_exportperson = array();
  var $_exportmatchplayer = array();
    var $_exportsinglematch = array();
  
  
function getTmpFiles()
{
$dir = JPATH_SITE.DS.'tmp'.DS.'ringerdateien';
$files = JFolder::files($dir, '^Termine', false, false, array('^Termine_Schema') );

foreach ( $files as $key => $value )
{
if ( preg_match("/Schema/i", $value ) )
{
unset($files[$key]);
}
}

//$this->dump_variable("files", $files);
return $files;
}
	...
	private function _getXml()
	{
		...
		{
			JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR', 'Missing import file'));
			echo "<script> alert('".JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR', 'Missing import file')."'); window.history.go(-1); </script>\n";
		}
	}

  private function _getXmlSingleMatch($singlematch)
	{
		if (JFile::exists(JPATH_SITE.DS.'tmp'.DS.'ringerdateien'.DS. $singlematch))
		{
			if (function_exists('simplexml_load_file'))
			{
				return @simplexml_load_file(JPATH_SITE.DS.'tmp'.DS.'ringerdateien'.DS. $singlematch,'SimpleXMLElement',LIBXML_NOCDATA);
			}
			else
			{
				JError::raiseWarning(500,JText::_('<a href="http://php.net/manual/en/book.simplexml.php" target="_blank">SimpleXML</a> does not exist on your system!'));
...
$tempexportclubs = array();
$tempexportplayground = array();
$tempexportrounds = array();
$tempexportreferee = array();
...
$temp->exportDate = '2010-09-19';
$temp->exportTime = '23:00:00';
$temp->exportSystem = 'LIGA-DB';
...
$temp = new stdClass();
$temp->id = 10000;
$temp->name = 'Kampfrichter';
$temp->alias = 'Kampfrichter';
$temp->parent_id = 1;
$temp->published = 1;
$temp->persontype = 3;
$exportposition[] = $temp;
...
// gewichtsklassen
$query = "SELECT *
FROM #__joomleague_position_ringen";
$this->_db->setQuery( $query );
$exportposition = $this->_db->loadObjectList();

foreach ( $exportposition as $row )
{
$row->parent_id = 2;
$row->published = 1;
$row->persontype = 1;
...
$temp = new stdClass();
$temp->id = $row->short_name; 
$temp->value = $row->id;
$this->_playerposition[] = $temp;
...
// echo '<pre>';
// print_r($this->_playerposition);
// echo '</pre>';
  
      		
    foreach ( $xmlData->Table as $row )
		{
    // liga
    $temp = new stdClass();
    $temp->id = 1;
    $temp->name = $row->Ligabezeichnung;
    $temp->country = 'DEU';
    ...
    $temp = new stdClass();
    $temp->id = 1;
    $temp->name = $row->Saison;
    $this->_datas['season'] = $temp;
    
    // projekt
    $temp = new stdClass();
    $temp->id = $row->SaisonLigaID; 
    $temp->sports_type_id = 1;
    $temp->name = $row->Ligabezeichnung.' '.$row->Saison;
    $temp->serveroffset = 0;
    ...
    // schiedsrichter
    if ( array_key_exists(trim((string)$row->KAMPFRICHT),$tempexportreferee) || trim((string)$row->KAMPFRICHT) == '' )
    	{
    // nichts machen
    }
    else
    {
    $tempexportreferee[(string)$row->KAMPFRICHT] = $lfdreferee;
    $temp = new stdClass();
    $temp->person_id = $this->_person_id;
    $temp->id = $lfdreferee;
    $temp->project_id = (int)$row->SaisonLigaID;
    $temp->project_position_id = 10001;
    $exportreferee[] = $temp;
    
    $temp = new stdClass();
    $temp->id = $this->_person_id;
    // den namen herausfinden
    $order   = array("(", ",", "/", " - ");
    $replace = array("#", "#", "#", "-");
    $newstr = str_replace($order, $replace, (string)$row->KAMPFRICHT);
    // mehrere leerzeichen in 1 leerzeichen abändern
    $newstr = preg_replace('/\\s\\s+/', ' ', $newstr);
    $vornachname = explode("#",$newstr);
    $teilname = explode(" ",(string)$vornachname[0]);
        
    //$temp->lastname = (string)$row->KAMPFRICHT;
    if ( sizeof($teilname) > 1 )
    {
    $temp->lastname = trim( $teilname[1] );
    $temp->firstname = trim( $teilname[0] );
    }
    else
    {
    $temp->lastname = trim( $teilname[0] );
    $temp->firstname = '';
    }
    
    $temp->alias = JFilterOutput::stringURLSafe( (string)$row->KAMPFRICHT );
    $temp->country = 'DEU';
    $temp->nickname = '';
    ...
    // spieltage
    if (array_key_exists((int)$row->KampftagePosIDIst,$tempexportrounds))
		{
    // nichts machen
    }
    else
    {
    $tempexportrounds[(int)$row->KampftagePosIDIst] = $row->KampftagePosIDIst;
    $temp = new stdClass();
    $temp->name = (int)$row->KampftagePosIDIst.'. Kampftag';
    $temp->roundcode = (int)$row->KampftagePosIDIst;
    $temp->id = (int)$row->KampftagePosIDIst;
    $exportrounds[] = $temp;
    }
    
    // sporthallen
    if (array_key_exists((int)$row->AusrichterID,$tempexportplayground))
		{
    // nichts machen
    }
    else
    {
    $tempexportplayground[(int)$row->AusrichterID] = (string)$row->Wettkampfstaette;
    $temp = new stdClass();
    // den namen der sporthalle richtig setzen
    $teile = explode(",",(string)$row->Wettkampfstaette);
    $temp->name = trim( $teile[0] );
    $temp->alias = JFilterOutput::stringURLSafe( (string)$row->Wettkampfstaette );
    $temp->short_name = '';
    $temp->id = (int)$row->AusrichterID;
    $exportplayground[] = $temp;
    }
    
    // heimmannschaft
    if (array_key_exists((int)$row->MannschaftenIDHeim,$tempexportclubs))
		{
    // nichts machen
    }
    else
    {
    $tempexportclubs[(int)$row->MannschaftenIDHeim] = (string)$row->Heimmannschaft;
    $temp = new stdClass();
    $temp->name = trim( (string)$row->Heimmannschaft );
    $temp->alias = JFilterOutput::stringURLSafe( (string)$row->Heimmannschaft );
    $temp->id = (int)$row->MannschaftenIDHeim;
    $temp->info = '';
    $temp->extended = '';
    $temp->standard_playground = (int)$row->AusrichterID;
    $temp->country = 'DEU';
    $exportclubs[] = $temp;
    ...
    $temp->name = trim( (string)$row->Heimmannschaft );
    $temp->alias = JFilterOutput::stringURLSafe( (string)$row->Heimmannschaft );
    $temp->short_name = trim( (string)$row->Heimmannschaft );
    $temp->middle_name = trim( (string)$row->Heimmannschaft );
    $temp->club_id = (int)$row->MannschaftenIDHeim;
    $temp->id = (int)$row->MannschaftenIDHeim;
    $temp->info = '';
    $temp->extended = '';
    $exportteams[] = $temp;
    ...
    $temp = new stdClass();
    $temp->team_id = (int)$row->MannschaftenIDHeim;
    $temp->id = (int)$row->MannschaftenIDHeim;
    $temp->extended = '';
    $temp->is_in_score = 1;
    $temp->standard_playground = (int)$row->AusrichterID;
    ...
    // gastmannschaft
    if (array_key_exists((int)$row->MannschaftenIDGast,$tempexportclubs))
		{
    // nichts machen
    }
    else
    {
    $tempexportclubs[(int)$row->MannschaftenIDGast] = (string)$row->Gastmannschaft;
    $temp = new stdClass();
    $temp->name = trim( (string)$row->Gastmannschaft );
    $temp->alias = JFilterOutput::stringURLSafe( (string)$row->Gastmannschaft );
    $temp->id = (int)$row->MannschaftenIDGast;
    $temp->info = '';
    $temp->extended = '';
    $temp->standard_playground = (int)$row->AusrichterID;
    $temp->country = 'DEU';
    ...
    $temp->name = trim( (string)$row->Gastmannschaft );
    $temp->alias = JFilterOutput::stringURLSafe( (string)$row->Gastmannschaft );
    $temp->short_name = trim( (string)$row->Gastmannschaft );
    $temp->middle_name = trim( (string)$row->Gastmannschaft );
    $temp->club_id = (int)$row->MannschaftenIDGast;
    $temp->id = (int)$row->MannschaftenIDGast;
    $temp->info = '';
    $temp->extended = '';
    $exportteams[] = $temp;
    ...
    $temp = new stdClass();
    $temp->team_id = (int)$row->MannschaftenIDGast;
    $temp->id = (int)$row->MannschaftenIDGast;
    $temp->extended = '';
    $temp->is_in_score = 1;
    $temp->project_id = (int)$row->SaisonLigaID;
    ...
    // das spiel
    $temp = new stdClass();
    $temp->id = (int)$row->BegegnungsID;
    $temp->projectteam1_id = (int)$row->MannschaftenIDHeim;
    $temp->projectteam2_id = (int)$row->MannschaftenIDGast;
    $temp->playground_id = (int)$row->AusrichterID;
    $temp->round_id = (int)$row->KampftagePosIDIst;
    $temp->match_number = str_pad( (int)$row->BegegnungsID, 8, "0", STR_PAD_LEFT);
    $temp->team1_result = (int)$row->PunkteHeimMeldung;
    $temp->team2_result = (int)$row->PunkteGastMeldung;
    $temp->extended = '';
    $temp->published = 1;
    $temp->count_result = 1;
    $temp->crowd = (int)$row->Zuschauer;
    $datetime = strtotime( (string)$row->KampftagIst );
    $match_date = date('Y-m-d', $datetime);
    $match_time = date('H:i', $datetime);
    $temp->match_date = $match_date." ".$match_time;
    $exportmatch[] = $temp;
    
    // gibt es zu der paarung schon einzelkämpfe ?
    $this->_getSingleMatch($temp->match_number,$temp->projectteam1_id,$temp->projectteam2_id,$temp->id,(int)$row->KampftagePosIDIst);
   
    // schiedsrichter dem spiel zuordnen
    if (array_key_exists(trim((string)$row->KAMPFRICHT),$tempexportreferee))
    {
    $temp = new stdClass();
    $temp->id = (int)$row->BegegnungsID;
    $temp->match_id = (int)$row->BegegnungsID;
    $temp->project_referee_id = $tempexportreferee[trim((string)$row->KAMPFRICHT)];
    $temp->project_position_id = 1001;
    $exportmatchreferee[] = $temp;
    }
    ...
    */
    ...
    $this->_datas['round'] = array_merge($exportrounds);                              
    ...
    $this->_datas['person'] = array_merge($exportperson);

    if ( isset($this->_exportperson) )
    {
    $this->_datas['person'] = array_merge($this->_exportperson);
    }
    
    if ( isset($this->_tempexportteamplayer) )
    {
    $this->_datas['teamplayer'] = array_merge($this->_tempexportteamplayer);
    }
    
    if ( isset($this->_exportmatchplayer) )
    {
    $this->_datas['matchplayer'] = array_merge($this->_exportmatchplayer);
    }
    
    if ( isset($this->_exportsinglematch) )
    {
    $this->_datas['singlematch'] = array_merge($this->_exportsinglematch);
    echo 'data->singlematch<pre>';
    print_r($this->_datas['singlematch']);
    echo '</pre>';
    }
    ...
    */
	...
	{
		$query  = ' SELECT t.id AS value, CONCAT(c.name, " - ", t.name) AS text, c.name AS club_name, t.name AS team_name, c.country'
	...
	{
		$query='SELECT id,name,short_name,club_id,alias FROM #__joomleague_playground ORDER BY name';
		$this->_db->setQuery($query);
		return $this->_db->loadObjectList();
	}
...
//$this->dump_variable("this->_dbpositionsid", $this->_dbpositionsid);
		if (!empty($this->_dbpositionsid))
		{
			foreach ($this->_dbpositionsid AS $key => $id)
			{
				...
				$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
				$my_text .= JText::sprintf(	'JL_ADMIN_EXTENSION_IMPORTING_GENERAL_POSITION_DATA0',
											'</span><strong>'.JText::_($this->_getObjectName('position',$id)).'</strong>');
				$my_text .= '<br />';
			}
		}
...
//$this->dump_variable("import_position", $import_position);
				...
				if ($this->_db->getAffectedRows())
				{
					...
					if ($this->_convertPositionID[$oldID]!=$p_position->id)
					{
						...
						$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
						$my_text .= JText::sprintf('JL_ADMIN_EXTENSION_IMPORTING_GENERAL_POSITION_DATA1','</span><strong>'.JText::_($p_position->name).'</strong>');
						$my_text .= '<br />';
					}
				}
				else
				{
					if ($p_position->store()===false)
					{
						...
						$this->_success_text['JL_ADMIN_EXTENSION_IMPORTING_GENERAL_POSITION_DATA2']=$my_text;
						return false;
					}
					else
					{
						...
						$my_text .= '<span style="color:'.$this->storeSuccessColor.'">';
						$my_text .= JText::sprintf('JL_ADMIN_EXTENSION_IMPORTING_GENERAL_POSITION_DATA4','</span><strong>'.JText::_($p_position->name).'</strong>');
						$my_text .= '<br />';
					}
				}
//$this->dump_variable("p_position", $p_position);
...
//$this->dump_variable("this->_convertPositionID", $this->_convertPositionID);
		$this->_success_text['JL_ADMIN_EXTENSION_IMPORTING_GENERAL_POSITION_DATA3']=$my_text;
		return true;
	}
...
//$this->dump_variable("this->_newclubsid", $this->_newclubsid);
		...
		if (!empty($this->_dbclubsid))
		{
			...
			foreach ($this->_dbclubsid AS $key => $id)
			{
				if (empty($this->_newclubs[$key]))
				{
					...
					$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
					$my_text .= JText::sprintf(	'JL_ADMIN_EXTENSION_IMPORTING_GENERAL_CLUB_DATA',
...
//$this->dump_variable("this->_newclubs", $this->_newclubs);
				...
				if ($object=$this->_db->loadObject())
				{
					...
					$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
					$my_text .= JText::sprintf('JL_ADMIN_EXTENSION_IMPORTING_GENERAL_CLUB_DATA0',"</span><strong>$object->name</strong>");
					$my_text .= '<br />';
				}
				else
				{
...
//$this->dump_variable("p_team", $p_team);
			}
		}
		$this->_success_text['Importing general team data:']=$my_text;
		return true;
	...
	private function _importPersons()
	{
		...
		if (!empty($this->_dbpersonsid))
		{
			foreach ($this->_dbpersonsid AS $key => $id)
			{
				...
				$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
				$my_text .= JText::sprintf(	'JL_ADMIN_EXTENSION_IMPORTING_GENERAL_PERSON_DATA',
		...
		if (!empty($this->_newpersonsid))
		{
			foreach ($this->_newpersonsid AS $key => $id)
			{
				...
				if ($object=$this->_db->loadObject())
				{
					...
					$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
					$my_text .= JText::sprintf(	'JL_ADMIN_EXTENSION_IMPORTING_GENERAL_PERSON_DATA0',
				...
				else
				{
					if ($p_person->store()===false)
					{
						...
						$this->_success_text['JL_ADMIN_EXTENSION_IMPORTING_GENERAL_PERSON_DATA1']=$my_text;
						return false;
					}
					else
					{
						...
						$my_text .= '<span style="color:'.$this->storeSuccessColor.'">';
						$my_text .= JText::sprintf('JL_ADMIN_EXTENSION_CREATED_NEW_PERSON_DATA: %1$s',"</span><strong>$dNameStr</strong>");
						$my_text .= '<br />';
					}
				}
			}
		}
		$this->_success_text['JL_ADMIN_EXTENSION_IMPORTING_GENERAL_PERSON_DATA2']=$my_text;
		return true;
	}
...
//$this->dump_variable("this->_convertTeamStaffID", $this->_convertTeamStaffID);
    ...
    private function _importSingleMatches()
	{
//$this->dump_header("function _importMatches");
		$my_text='';
		if (!isset($this->_datas['singlematch']) || count($this->_datas['singlematch'])==0){return true;}

		if (!isset($this->_datas['team']) || count($this->_datas['team'])==0){return true;}
		if ((!isset($this->_newteams) || count($this->_newteams)==0) &&
			(!isset($this->_dbteamsid) || count($this->_dbteamsid)==0)){return true;}	   

foreach ($this->_datas['singlematch'] as $key => $match)
{
//$this->dump_variable("singlematch", $match);
			$p_match =& $this->getTable('jlextindividualsport');
			$oldId = (int)$match->id;
            $oldMatchID = $this->_getDataFromObject($match,'match_id');
			$oldTeamPlayer1ID = $this->_getDataFromObject($match,'teamplayer1_id');
            $oldTeamPlayer2ID = $this->_getDataFromObject($match,'teamplayer2_id');
...
$p_match->set('match_id',$this->_convertMatchID[$oldMatchID]);

$newmatchid = $this->_convertMatchID[$oldMatchID];

$p_match->set('teamplayer1_id',$this->_convertTeamPlayerID[$oldTeamPlayer1ID]);
$p_match->set('teamplayer2_id',$this->_convertTeamPlayerID[$oldTeamPlayer2ID]);
...
if ($p_match->store()===false)
			{
				...
				$my_text .= $oldID;
				$my_text .= "<br />Error: _importSingleMatches<br />#$my_text#<br />#<pre>".print_r($p_match,true).'</pre>#';
				$this->_success_text['Importing single match data:']=$my_text;
				return false;
			}
            else
            {
            if ($this->import_version=='NEW')
				{
					...
					$my_text .= '<span style="color:'.$this->storeSuccessColor.'">';
					$my_text .= JText::sprintf(	'Added Single Match to round: %1$s / Match: %2$s - %3$s',
...
$rowupdate->load( $newmatchid );
$rowupdate->team1_result = $rowupdate->team1_result + $team1_result;
$rowupdate->team2_result = $rowupdate->team2_result + $team2_result;
if (!$rowupdate->store()) 
{
...
//$this->dump_variable("import_matchplayer", $import_matchplayer);
			...
			{
				$my_text .= '<span style="color:red">';
				$my_text .= JText::sprintf(	'JL_ADMIN_EXTENSION_IMPORTING_GENERAL_MATCHPLAYER_DATA2',
								"</span><strong>$oldID</strong><span style='color:red'>",
			...
			if ($p_matchplayer->store()===false)
			{
				...
				$this->_success_text['JL_ADMIN_EXTENSION_IMPORTING_GENERAL_MATCHPLAYER_DATA0:']=$my_text;
				return false;
			}
			else
			{
				...
				$my_text .= '<span style="color:'.$this->storeSuccessColor.'">';
				$my_text .= JText::sprintf(	'JL_ADMIN_EXTENSION_IMPORTING_GENERAL_MATCHPLAYER_DATA3',
								'</span><strong>'.$p_matchplayer->match_id.'</strong><span style="color:'.$this->storeSuccessColor.'">',
								'</span><strong>'.$dPerson->lastname,
								$dPerson->firstname.'</strong><span style="color:'.$this->storeSuccessColor.'">',
								"</span><strong>$dPosName</strong>");
				$my_text .= '<br />';
				$my_text .= JText::sprintf(	'JL_ADMIN_EXTENSION_IMPORTING_GENERAL_MATCHPLAYER_DATA4',
								"</span><strong>$oldMatchID</strong>");
				$my_text .= '<br />';
			}
//$this->dump_variable("p_matchplayer", $p_matchplayer);
		}
		$this->_success_text['JL_ADMIN_EXTENSION_IMPORTING_GENERAL_MATCHPLAYER_DATA1']=$my_text;
		return true;
	}
	...
	public function importData($post)
	{
		...
		if (is_array($post) && count($post) > 0)
		{
			...
			if ($post['importProject'])
			{
				...
				{
					JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Missing projectname'));
					echo "<script> alert('".JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Missing projectname')."'); window.history.go(-1); </script>\n";
				}

				if (empty($this->_datas['project']))
				{
					JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Project object is missing inside import file!!!'));
					echo "<script> alert('".JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Project object is missing inside import file!!!')."'); window.history.go(-1); </script>\n";
					return false;
				}

				if ($this->_checkProject()===false)
				{
					JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Projectname already exists'));
					echo "<script> alert('".JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Projectname already exists')."'); window.history.go(-1); </script>\n";
					return false;
				}
			}
			...
			if ($post['importProject'] || $post['importType']=='events' || $post['importType']=='positions')
			{
				...
				elseif (isset($post['sportstypeNew']))
					...
					{
						JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Missing sportstype'));
						echo "<script> alert('".JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Missing sportstype')."'); window.history.go(-1); </script>\n";
						return false;
					}
			}
			...
			if ($post['importProject'])
			{
				...
				elseif (isset($post['leagueNew']))
					...
					{
						JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Missing league'));
						echo "<script> alert('".JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Missing league')."'); window.history.go(-1); </script>\n";
						return false;
					}
				...
				elseif (isset($post['seasonNew']))
					...
					{
						JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Missing season'));
						echo "<script> alert('".JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Missing season')."'); window.history.go(-1); </script>\n";
						return false;
					}
			...
			if ($post['importProject'] || $post['importType']=='events' || $post['importType']=='positions')
			{
				...
				{
					JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_ERROR_DURING','sports-type'));
			...
			if ($post['importProject'])
			{
				...
				{
					JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_ERROR_DURING','league'));
					return $this->_success_text;
				}
				...
				{
					JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_ERROR_DURING','season'));
					return $this->_success_text;
				}
			}
...
// TO BE FIXED: Check correct function
            ...
            if ($this->_importSingleMatches()===false)
			{
				JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_ERROR_DURING','singlematch'));
				return $this->_success_text;
			}
			...
			{
				JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_ERROR_DURING','treeto'));
				return $this->_success_text;
			}
			...
			{
				JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_ERROR_DURING','treetonode'));
				return $this->_success_text;
			}
			...
			{
				JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_ERROR_DURING','treetomatch'));
				return $this->_success_text;
			}
      ...
      $this->_SetRoundDates($this->_project_id); 
		...
		{
			$this->_deleteImportFile();
			JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR','Missing import data'));
			return false;
		}
	}
...
/*
* jetzt noch das datum der runden von bis setzen
*/
function setRoundsDate($rounds)
{
global $mainframe, $option;
...
$mainframe->enqueueMessage('setRoundsDate datum in den runden setzen', '');

$this->_id =  $mainframe->getUserState( $option . 'project', 0 );

/*
// formatierte Ausgabe
echo 'array setRoundsDate <br>';
...
//echo 'setRoundsDate von ->'.$von.' bis -> '.$bis.'<br>';
...
/*
echo 'error array setRoundsDate <br>';
echo '<pre>';
...
*/
  ...
  function _getSingleMatch($match_number,$projectteam1_id,$projectteam2_id,$match_id,$round_id)
  {
  global $mainframe, $option;
  $document	=& JFactory::getDocument();
  $mainframe	=& JFactory::getApplication();
...
$awayresult = array();
  ...
  $xmlSingleData = $this->_getXmlSingleMatch($files[0]);
  
//   echo '<pre>';
//   print_r($xmlSingleData);
//   echo '</pre>';
  
  foreach ( $xmlSingleData->Table as $row )
		{
		
	// lizenznummer prüfen
    if ( empty( $row->HLizenzNr ) )
    {
    $mainframe->enqueueMessage(JText::_('Ringer '.trim( (string)$row->HVName ) .'-'. trim( (string)$row->HFName).' keine lizenz match -> '.$match_number ),'Error');        
    }
    if ( empty( $row->GLIZENZNR ) )
    {
    $mainframe->enqueueMessage(JText::_('Ringer '.trim( (string)$row->GVName ) .'-'. trim( (string)$row->GFNAME).' keine lizenz match -> '.$match_number ),'Error');        
    }
    
    // heimspieler
    if ( array_key_exists((int)$row->HLizenzNr,$this->_tempexportplayer) || (int)$row->HLizenzNr == '' )
    {
    // nichts machen
    // aber match player anlegen
    
    // position finden
    $possuchen = trim( (string)$row->Stilart ) . trim( (string)$row->Gewichtsklasse );
    foreach ( $this->_playerposition as $key )
    {
    
    if ( $key->id == $possuchen )
    {
    $temp->position_id = $key->value;
    $project_position_id = $key->value; 
    break;
    }
    else
    {
    $temp->position_id = 0;
    $project_position_id = 0;
    }
    }
    $temp = new stdClass();
    $temp->id = $this->_matchplayer_id;
    $temp->project_position_id = $project_position_id;
    $temp->match_id = $match_id;
    $temp->teamplayer_id = $this->_tempexportplayer[ (int)$row->HLizenzNr ];
    $temp->in_for = 0;
    $teamplayer1_id = $temp->teamplayer_id;
    $this->_exportmatchplayer[] = $temp;
    $this->_matchplayer_id++;
    
    if ( !$project_position_id )
    {
    $mainframe->enqueueMessage(JText::_('Ringer '.trim( (string)$row->HVName ) .'-'. trim( (string)$row->HFName).' hat keine Position -> '.$possuchen ),'Error');
    }
    ...
    {
    $project_position_id = 0;
	$this->_tempexportplayer[ (int)$row->HLizenzNr ] = $this->_person_id;
	$temp = new stdClass();
    $temp->id = $this->_person_id;
    $temp->lastname = trim( (string)$row->HFName );
    $temp->firstname = trim( (string)$row->HVName );
    $temp->alias = JFilterOutput::stringURLSafe( trim( (string)$row->HVName ) .'-'. trim( (string)$row->HFName ) );
    $temp->country = 'DEU';
    $temp->nickname = '';
    $temp->birthday = '0000-00-00';
    $temp->info = trim( (string)$row->Stilart ) . trim( (string)$row->Gewichtsklasse );
    $temp->knvbnr = (int)$row->HLizenzNr;
    $temp->extended = '';
    // position finden
    $possuchen = trim( (string)$row->Stilart ) . trim( (string)$row->Gewichtsklasse );
    ...
    $this->_exportperson[] = $temp;
    
    if ( !$project_position_id )
    {
    $mainframe->enqueueMessage(JText::_('Gewichtsklasse '.$possuchen.' nicht vorhanden' ),'Error');
    }
    
    $temp = new stdClass();
    $temp->id = $this->_person_id;
    $temp->projectteam_id = $projectteam1_id;
    $temp->project_position_id = $project_position_id;
    $temp->person_id = $this->_person_id;
    $this->_tempexportteamplayer[] = $temp;
    
    $temp = new stdClass();
    $temp->id = $this->_matchplayer_id;
    $temp->project_position_id = $project_position_id;
    $temp->match_id = $match_id;
    $temp->teamplayer_id = $this->_person_id;
    $temp->in_for = 0;
    $teamplayer1_id = $temp->teamplayer_id;
    $this->_exportmatchplayer[] = $temp;
	...
	// gastspieler anfang
    if ( array_key_exists((int)$row->GLIZENZNR,$this->_tempexportplayer) || (int)$row->GLIZENZNR == '' )
    {
    // nichts machen
    // aber match player anlegen
    ...
    foreach ( $this->_playerposition as $key )
    {
    //echo 'schluessel : '.$key->id.' value :'.$key->value. ' suchen :'. $possuchen .'<br>';
    if ( $key->id == $possuchen )
    {
    //echo 'gefunden<br>';
    $temp->position_id = $key->value;
    ...
    $temp->project_position_id = $project_position_id;
    $temp->match_id = $match_id;
    $temp->teamplayer_id = $this->_tempexportplayer[ (int)$row->GLIZENZNR ];
    $temp->in_for = 0;
    $teamplayer2_id = $temp->teamplayer_id;
    $this->_exportmatchplayer[] = $temp;
    $this->_matchplayer_id++;
    
    if ( !$project_position_id )
    {
    $mainframe->enqueueMessage(JText::_('Ringer '.trim( (string)$row->GVName ) .'-'. trim( (string)$row->GFNAME).' hat keine Position -> '.$possuchen ),'Error');
    }
    ...
    {
    $project_position_id = 0;
	$this->_tempexportplayer[ (int)$row->GLIZENZNR ] = $this->_person_id;
	$temp = new stdClass();
    $temp->id = $this->_person_id;
    $temp->lastname = trim( (string)$row->GFNAME );
    $temp->firstname = trim( (string)$row->GVName );
    $temp->alias = JFilterOutput::stringURLSafe( trim( (string)$row->GVName ) .'-'. trim( (string)$row->GFNAME ) );
    $temp->country = 'DEU';
    $temp->nickname = '';
    $temp->birthday = '0000-00-00';
    $temp->info = trim( (string)$row->Stilart ) . trim( (string)$row->Gewichtsklasse );
    $temp->knvbnr = (int)$row->GLIZENZNR;
    $temp->extended = '';
    // position finden
    $possuchen = trim( (string)$row->Stilart ) . trim( (string)$row->Gewichtsklasse );
    ...
    $temp = new stdClass();
    $temp->id = $this->_person_id;
    $temp->projectteam_id = $projectteam2_id;
    $temp->project_position_id = $project_position_id;
    $temp->person_id = $this->_person_id;
    $this->_tempexportteamplayer[] = $temp;
	...
	// und jetzt die einzelpaarung
    $homeresult[] = (int)$row->TPR1;
    $awayresult[] = (int)$row->TPB1;
    $homeresult[] = (int)$row->TPR2;
    $awayresult[] = (int)$row->TPB2;
    $homeresult[] = (int)$row->Expr9;
    $awayresult[] = (int)$row->Expr10;
    $homeresult[] = (int)$row->TPR4;
    $awayresult[] = (int)$row->TPB4;
    $homeresult[] = (int)$row->TPR5;
    $awayresult[] = (int)$row->TPB5;
    // das spiel
    $temp = new stdClass();
    $temp->id = $this->_lfdsinglematch;
    $temp->projectteam1_id = $projectteam1_id;
    $temp->projectteam2_id = $projectteam2_id;
    $temp->teamplayer1_id = $teamplayer1_id;
    $temp->teamplayer2_id = $teamplayer2_id;
    $temp->round_id = $round_id;
    $temp->match_id = $match_id;
    $temp->team1_result_split = implode(';',$homeresult);
    $temp->team2_result_split = implode(';',$awayresult);
    $temp->team1_result = (string)$row->HPunkte;
    $temp->team2_result = (string)$row->GPUNKTE;

    $temp->match_number = $match_number;
    $temp->published = 1;
    $temp->count_result = 1;
    $temp->show_report = 1;
    $temp->summary = (string)$row->PUNKTEVER;
    $this->_exportsinglematch[] = $temp;	
	$this->_lfdsinglematch++;
    unset($homeresult);
    unset($awayresult);
