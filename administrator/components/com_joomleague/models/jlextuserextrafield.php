 ...
 * @since	0.1
 */
class JoomleagueModeljlextuserextrafield extends JoomleagueModelItem
{
	/**
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query='SELECT * FROM #__joomleague_user_extra_fields WHERE id='.(int) $this->_id;
			$this->_db->setQuery($query);
	...
	function _initData()
	{
		...
		if (empty($this->_data))
		{
			...
			$season->id					= 0;
			$season->name				= null;
			$season->template_backend				= null;
			$season->checked_out		= 0;
			$season->checked_out_time	= 0;
			$season->template_frontend			= null;
			$season->ordering			= 0;
	...
	function addSeason($newseason)
	{
		...
		$query="SELECT * FROM #__joomleague_user_extra_fields WHERE name='$newseason'";
		$this->_db->setQuery($query);
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'jlextuserextrafield', $prefix = 'table', $config = array())
	{
