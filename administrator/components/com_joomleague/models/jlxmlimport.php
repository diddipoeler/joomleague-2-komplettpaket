 ...
 * @since	1.5.0a
 */
class JoomleagueModelJLXMLImport extends JModel
{
	var $_datas=array();
	var $_league_id=0;
	var $_season_id=0;
	...
	private function _getXml()
	{
		...
		{
			JError::raiseWarning(500,JText::sprintf('COM_JOOMLEAGUE_ADMIN_XML_IMPORT_ERROR', 'Missing import file'));
		...
		if ((isset($xmlData->record)) && (is_object($xmlData->record)))
		{
			foreach ($xmlData->record as $value)
			{
				...
				if ($xmlData->record[$i]['object']=='JoomLeague')
				{
					...
					JError::raiseNotice(0,JText::_('COM_JOOMLEAGUE_ADMIN_XML_IMPORT_RENDERING_093'));
				}
				...
				if ($xmlData->record[$i]['object']=='JoomLeague15')
				{
					...
					JError::raiseNotice(500,JText::_('COM_JOOMLEAGUE_ADMIN_XML_IMPORT_RENDERING_15'));
				}
				...
				if ($xmlData->record[$i]['object']=='JoomLeague20')
				{
					$this->_datas['project']=$xmlData->record[$i];
					$this->import_version='NEW';
					JError::raiseNotice(500,JText::_('COM_JOOMLEAGUE_ADMIN_XML_IMPORT_RENDERING_20'));
				}
...
//die();
		...
		{
			JError::raiseWarning(500,JText::sprintf('COM_JOOMLEAGUE_ADMIN_XML_IMPORT_ERROR', 'Something is wrong inside the import file'));
			return false;
		}
	}
