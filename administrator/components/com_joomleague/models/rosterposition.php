 ...
 * @since	0.1
 */
class JoomleagueModelrosterposition extends JoomleagueModelItem
{
	/**
	...
	function delete($cid=array())
	{
      ...
      */
			$query="DELETE FROM #__joomleague_rosterposition WHERE id IN ($cids)";
			$this->_db->setQuery($query);
			if (!$this->_db->query())
			{
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query='SELECT * FROM #__joomleague_rosterposition WHERE id='.(int) $this->_id;
			$this->_db->setQuery($query);
	...
	function addLeague($newLeague)
	{
		...
		$query="SELECT name FROM #__joomleague_rosterposition WHERE name='$newLeague'";
		$this->_db->setQuery($query);
  ...
  /**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'rosterposition', $prefix = 'table', $config = array())
	{
