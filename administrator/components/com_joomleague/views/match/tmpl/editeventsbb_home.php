			...
			for( $i=0 , $n = count( $this->homeRoster ); $i < $n; $i++ )
			{
					...
					<tr id="row<?php echo $i;?>">
						...
						foreach ( $this->events as $ev)
						{
                            ...
                            <input type="text" size="2" class="inputbox" name="notice_h_<?php echo $i.'_'.$tehp; ?>" value="<?php echo ((strlen($this->evbb[0]->notice) > 0) ? $this->evbb[0]->notice : '' ); ?>" title="<?php echo JText::_('COM_JOOMLEAGUE_ADMIN_MATCH_EE_MATCH_NOTICE' )?>" onchange="document.getElementById('cb_h<?php echo $i;?>').checked=true" />
                            &nbsp;&nbsp;
                            </td>
