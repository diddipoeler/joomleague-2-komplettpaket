...
function JLprintGMapsJS()
{
    $this->apiSensor ? $_sensor = "true" : $_sensor = "false";
    $return = '';
    //$return =  "\n<!-- Include Google Maps JS -->";
    $return .=  "http://maps.google.com/maps/api/js?sensor=$_sensor";
    return $return;
}
...
function JLshowMap($zoomToBounds = true)
{
    $javascript = '';
    $this->showDefaultUI ? $_disableDefaultUI = "false" : $_disableDefaultUI = "true";
    $this->showMapTypeControl ? $_mapTypeControl = "true" : $_mapTypeControl = "false";
...
//    echo "\n<div id=\"gmap_canvas\" style=\"width: ".$this->mapWidth."px; height: ".$this->mapHeight."px;\">\n</div>\n";
    ...
    $javascript .= "\n\n".
	 ...
	 "\tcenter: latlng,\n".
	 "\tmapTypeId: google.maps.MapTypeId.".$this->mapType.",\n";
    if ( $this->mapBackgroundColor != "" ) { $javascript .= "backgroundColor: '".$this->mapBackgroundColor."',\n"; }
    $javascript .= "\tdisableDefaultUI: ".$_disableDefaultUI.",\n".
         "\tmapTypeControl: ".$_mapTypeControl.",\n".
         "\tmapTypeControlOptions: { style: google.maps.MapTypeControlStyle.".$this->mapTypeControlStyle." },\n".
         "\tnavigationControl: ".$_navigationControl.",\n".
         "\tnavigationControlOptions: { style: google.maps.NavigationControlStyle.".$this->navigationControlStyle." },\n".
    ...
    if ( ($this->infoWindowBehaviour == 'CLOSE_ON_MAPCLICK') OR ($this->infoWindowBehaviour == 'SINGLE_CLOSE_ON_MAPCLICK') )
    {
        $javascript .= "\tgoogle.maps.event.addListener(map, 'click', function() { if (currentInfoWindow != null) { currentInfoWindow.close(); } });\n";
    }

    $javascript .= "google.maps.event.trigger(map, \"resize\");\n";
    /*
     * Run through the mapMarkers array to display markers on the map
     */
    for ( $count = 0; $count < sizeof($this->mapMarkers); $count++ )
...
*/
        ...
        $javascript .= "\tvar markerLatLng = new google.maps.LatLng(".$this->mapMarkers[$count]['lat'].", ".$this->mapMarkers[$count]['lng'].");\n".
             "\tvar myIcon$count = new google.maps.MarkerImage('".$this->mapMarkers[$count]['iconURL']."', null, null, null, new google.maps.Size(40,40));    \n".
             "\tvar marker$count = new google.maps.Marker({\n".
             "\t position: markerLatLng,\n".
             "\t title: '".$this->mapMarkers[$count]['tooltip']."',\n";
        if ( $this->mapMarkers[$count]['iconURL'] != "" )
        {
             //$javascript .= "\t icon: '".$this->mapMarkers[$count]['iconURL']."',\n";
             $javascript .= "\t icon: myIcon$count,\n";
        }
        if ( $this->mapMarkers[$count]['clickable'] == false )
        {
             $javascript .= "\t clickable: false,\n";
        }
        $javascript .= "\t map: map\n".
             "\t});\n";
        ...
        {
            // create InfoWindow
            $javascript .= "\tvar infowindowM$count = new google.maps.InfoWindow({\n".
                 "\t content: '".$this->mapMarkers[$count]['info']."'\n".
                 
                 "\t});\n";
            // add an event to the marker
            $javascript .= "\tgoogle.maps.event.addListener (marker$count, '$_infowindowtrigger', function() {\n";
            // infoWindowBehaviour     
            if ( ($this->infoWindowBehaviour == 'SINGLE') OR ($this->infoWindowBehaviour == 'SINGLE_CLOSE_ON_MAPCLICK') )
            {
                $javascript .= "\t if (currentInfoWindow != null) { currentInfoWindow.close(); } \n";
            }
            $javascript .= "\t infowindowM$count.open(map, marker$count);\n".
                 "\t currentInfoWindow = infowindowM$count;\n".
                 "\t});\n";
        }
        $javascript .= "\tbounds.extend(markerLatLng);\n\n";
    }
    ...
    {
        // place the circle on the map
        $javascript .= "\tvar circleLatLng = new google.maps.LatLng(".$this->mapCircles[$count]['lat'].", ".$this->mapCircles[$count]['lng'].");\n".
             "\tvar circle$count = new google.maps.Circle({\n".
             "\t center: circleLatLng,\n".
             "\t radius: ".$this->mapCircles[$count]['rad'].",\n";
        // check if there are options set for the circle     
        if ( $this->mapCircles[$count]['fillColor'] != "" )     { $javascript .= "\t fillColor: '".$this->mapCircles[$count]['fillColor']."',\n"; }
        if ( $this->mapCircles[$count]['fillOpacity'] != "" )   { $javascript .= "\t fillOpacity: ".$this->mapCircles[$count]['fillOpacity'].",\n"; }
        if ( $this->mapCircles[$count]['strokeColor'] != "" )   { $javascript .= "\t strokeColor: '".$this->mapCircles[$count]['strokeColor']."',\n"; }
        if ( $this->mapCircles[$count]['strokeOpacity'] != "" ) { $javascript .= "\t strokeOpacity: ".$this->mapCircles[$count]['strokeOpacity'].",\n"; }
        if ( $this->mapCircles[$count]['strokeWeight'] != "" )  { $javascript .= "\t strokeWeight: ".$this->mapCircles[$count]['strokeWeight'].",\n"; }
        if ( $this->mapCircles[$count]['clickable'] == false )  { $javascript .= "\t clickable: false,\n"; }
        $javascript .= "\t map: map\n".
             "\t});\n";
        ...
        {
            // create InfoWindow
            $javascript .= "\tvar infowindowC$count = new google.maps.InfoWindow({\n".
                 "\t content: '".$this->mapCircles[$count]['info']."',\n".
                 "\t position: circleLatLng,\n".
                 "\t});\n";
            // add an event to the marker
            $javascript .= "\tgoogle.maps.event.addListener (circle$count, '$_infowindowtrigger', function() {\n";
            // infoWindowBehaviour     
            if ( ($this->infoWindowBehaviour == 'SINGLE') OR ($this->infoWindowBehaviour == 'SINGLE_CLOSE_ON_MAPCLICK') )
            {
                $javascript .= "\t if (currentInfoWindow != null) { currentInfoWindow.close(); } \n";
            }
            $javascript .= "\t var tmplat1 = circle$count.getCenter().lat()+(circle$count.getBounds().getNorthEast().lat() - circle$count.getCenter().lat())/2;\n";
            $javascript .= "\t var tmplng1 = circle$count.getCenter().lng()+(circle$count.getBounds().getNorthEast().lng() - circle$count.getCenter().lng())/2;\n";
            $javascript .= "\t var newpos = new google.maps.LatLng(tmplat1, tmplng1);\n";
            $javascript .= "\t infowindowC$count.open(map);\n".
                 "\t infowindowC$count.setPosition(newpos);\n".
                 "\t currentInfoWindow = infowindowC$count;\n".
                 "\t});\n";
        }

        $javascript .= "\tbounds.extend(circle$count.getBounds().getNorthEast());\n";
        $javascript .= "\tbounds.extend(circle$count.getBounds().getSouthWest());\n\n";
    }
    ...
    {
        // place the rectangle on the map
        $javascript .= "\tvar rectangleSW = new google.maps.LatLng(".$this->mapRectangles[$count]['lat1'].", ".$this->mapRectangles[$count]['lng1'].");\n".
             "\tvar rectangleNE = new google.maps.LatLng(".$this->mapRectangles[$count]['lat2'].", ".$this->mapRectangles[$count]['lng2'].");\n".
             "\tvar rectangleBounds = new google.maps.LatLngBounds(rectangleSW,rectangleNE);\n".
             "\tvar rectangle$count = new google.maps.Rectangle({\n".
             "\t bounds: rectangleBounds,\n";
        // check if there are options set for the rectangle     
        if ( $this->mapRectangles[$count]['fillColor'] != "" )     { $javascript .= "\t fillColor: '".$this->mapRectangles[$count]['fillColor']."',\n"; }
        if ( $this->mapRectangles[$count]['fillOpacity'] != "" )   { $javascript .= "\t fillOpacity: ".$this->mapRectangles[$count]['fillOpacity'].",\n"; }
        if ( $this->mapRectangles[$count]['strokeColor'] != "" )   { $javascript .= "\t strokeColor: '".$this->mapRectangles[$count]['strokeColor']."',\n"; }
        if ( $this->mapRectangles[$count]['strokeOpacity'] != "" ) { $javascript .= "\t strokeOpacity: ".$this->mapRectangles[$count]['strokeOpacity'].",\n"; }
        if ( $this->mapRectangles[$count]['strokeWeight'] != "" )  { $javascript .= "\t strokeWeight: ".$this->mapRectangles[$count]['strokeWeight'].",\n"; }
        if ( $this->mapRectangles[$count]['clickable'] == false )  { $javascript .= "\t clickable: false,\n"; }
        $javascript .= "\t map: map\n".
             "\t});\n";
        ...
        {
            // create InfoWindow
            $javascript .= "\tvar infowindowR$count = new google.maps.InfoWindow({\n".
                 "\t content: '".$this->mapRectangles[$count]['info']."',\n".
                 "\t position: rectangleNE,\n".
                 "\t});\n";
            // add an event to the marker
            $javascript .= "\tgoogle.maps.event.addListener (rectangle$count, '$_infowindowtrigger', function() {\n";
            // infoWindowBehaviour     
            if ( ($this->infoWindowBehaviour == 'SINGLE') OR ($this->infoWindowBehaviour == 'SINGLE_CLOSE_ON_MAPCLICK') )
            {
                $javascript .= "\t if (currentInfoWindow != null) { currentInfoWindow.close(); } \n";
            }
            $javascript .= "\t infowindowR$count.open(map);\n".
                 "\t currentInfoWindow = infowindowR$count;\n".
                 "\t});\n";
        }

        $javascript .= "\tbounds.extend(rectangleNE);\n";
        $javascript .= "\tbounds.extend(rectangleSW);\n\n";
    }
